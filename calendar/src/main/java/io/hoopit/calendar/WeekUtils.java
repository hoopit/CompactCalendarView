...
public class WeekUtils {
    ...
    static String[] getWeekdayNames(Locale locale, int day, boolean useThreeLetterAbbreviation) {
        ...
        for (int currentDay = day - 1, i = 0; i <= 6; i++, currentDay++) {
            ...
            //CHANGED Changed the letters to upper case so that they show in uppercase in norwegian as well.
            weekDayNames[i] = weekDaysFromSunday[currentDay].toUpperCase();
        }

        if (!useThreeLetterAbbreviation) {
            for (int i = 0; i < weekDayNames.length; i++) {
                //CHANGED Changed the letters to upper case so that they show in uppercase in norwegian as well.
                weekDayNames[i] = weekDayNames[i].substring(0, 1).toUpperCase();
            }
        }
